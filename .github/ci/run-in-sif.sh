...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-types's —
...
# so only scitex-types's own thin dep set is fetched per run.
#
...
# scitex-types's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-types + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-types's historical bare-uv/pip workflow so a
...
# scitex-types's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-types style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-types-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-types's heavy
